...
bool GraphViewer::addEdge(long id, long v1, long v2, long edgeType) {
	char buff[200];
	sprintf(buff, "addEdge %ld %ld %ld %ld\n", id, v1, v2, edgeType);
	string str(buff);
	return con->sendMsg(str);
}
...
bool GraphViewer::defineVertexSize(float size) {
	char buff[200];
	sprintf(buff, "defineVertexSize %f\n", size);
	string str(buff);
	return con->sendMsg(str);
}
